...
int main() {
    ...
    while(completed != n) {
    int idx = -1;
    int min_priority = INT_MAX; // Initialize min_priority with the maximum possible value
    for(int i = 0; i < n; i++) {
        if(p[i].arrival_time <= current_time && is_completed[i] == 0 && p[i].priority < min_priority) {
